"""Internal heatmap backend with variable row and column sizes.

This module contains the TableLens-style pcolormesh wrapper used by
``izzyviz.visualization``. It is intentionally internal; public visualization
APIs should live in ``visualization.py``.
"""
...
class _HeatMapper:
    ...
    def plot(self, ax, cax, kws):
        ...
        # Draw the heatmap
        mesh = ax.pcolormesh(
            X, Y, self.plot_data, cmap=self.cmap, norm=self.norm, **kws
        )
        ...
        if self.xtickevery == "auto":
            xticks, xticklabels = self._auto_ticks(
                ax, self.xticklabels, self.col_centers, axis=0
            )
        else:
            xticks, xticklabels = self._skip_ticks(
                self.col_centers, self.xticklabels, self.xtickevery
            )

        if self.ytickevery == "auto":
            yticks, yticklabels = self._auto_ticks(
                ax, self.yticklabels, self.row_centers, axis=1
            )
        else:
            yticks, yticklabels = self._skip_ticks(
                self.row_centers, self.yticklabels, self.ytickevery
            )
